...
describe('GreaterThan', function () {
    ...
    describe('#validate()', function () {
        it('is valid - number', function () {
            const object = new GreaterThan({value: 10});
        ...
        it('is valid - Date', function () {
            const object = new GreaterThan({value: new Date('2019-05-21')});
        ...
        it('is not valid - Number', function () {
            ...
            [-1, 9, 9.99999999, '7', false, true]
                .forEach((value) => {
                    ...
                    assert.strictEqual(e.message, 'This value should be greater than 10.');
                });
        });

        it('is not valid - Date', function () {
            const now = new Date('2019-12-05');
            const object = new GreaterThan({value: now});

            [new Date('2019-05-22'), '2019-05-22']
                .forEach((value) => {
                    const e = object.validate(value);

                    assert.strictEqual(e.message, 'This value should be greater than 12/5/2019.');
                });
        });
        ...
        it('is valid - strict', function () {
            [
                {value: 10, compare: 11},
                {value: 10.9999, compare: 11.9999},
                {value: '10', compare: '11'},
                {value: new Date('2019-05-22'), compare: new Date('2019-12-05')},
            ].forEach((pair) => {
                const object = new GreaterThan({value: pair.value, strict: true});
        ...
        it('is not valid - strict', function () {
            let object = new GreaterThan({value: 10, strict: true});
            ...
            object = new GreaterThan({value: 10, strict: true});
            e = object.validate('10.9999');

            assert.strictEqual(e.message, 'This values has different types. Given type is string; Expected type is number.');

            object = new GreaterThan({value: '11', strict: true});
            ...
            object = new GreaterThan({value: new Date(), strict: true});
            ...
            object = new GreaterThan({value: 'now', strict: true});
            e = object.validate(new Date());
